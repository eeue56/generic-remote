...
runWebCommand :: OutputGenerator
runWebCommand command xs = do

    let result = M.lookup command paths
    let (com, args) = durp result
    (_, stdOut, _, _) <- createProcess (proc com args) {std_out = CreatePipe}
...
runGetRequest args gen = do 
    ...
    f <- if M.member path paths then runWebCommand path getArgs else gen path getArgs
